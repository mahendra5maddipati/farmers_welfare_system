...
<!-- banner -->
    ...
    <br/>
    <h1 style="color:white;font-size: 50px; text-shadow: 3px 4px 2px black;" class="mx-auto d-block text-center"><?= _SEEDS ?></h1>
	</div>
<!-- //banner -->
<!-- services -->
	<div class="services">
		<div class="container">
			<p class="w3ls_para"><?= _SEEDS ?></p>
      <div class="w3l_services_grids">
        <div id="verticalTab">
          <ul class="resp-tabs-list">
            ...
            if(isset($_SESSION['lang'])){
                if ($_SESSION['lang'] == "en") {
                    $crops = "SELECT DISTINCT cropname_en FROM seeds_en ORDER BY created_on";
                } else {
                    $crops = "SELECT DISTINCT cropname_te FROM seeds_te ORDER BY created_on";
                }
            } else{
                $crops = "SELECT DISTINCT cropname_en FROM seeds_en ORDER BY created_on";
            }
            $distresult = $con->query($crops);
            if ($distresult && $distresult->num_rows > 0) {
                foreach ($distresult as $crop) {
                    if(isset($_SESSION['lang'])){
                        if ($_SESSION['lang'] == "en") {
                            echo "<li>" . $crop['cropname_en'] . "</li>";
                        } else {
                            echo "<li>" . $crop['cropname_te'] . "</li>";
                        }
                    } else{
                        echo "<li>" . $crop['cropname_en'] . "</li>";
                    }
          ...
          if ($distresult && $distresult->num_rows > 0) {
              foreach ($distresult as $district) {
                  ...
                  if(isset($_SESSION['lang'])){
                      if ($_SESSION['lang'] == "en") {
                          $cropname = $district['cropname_en'];
                          $seeds = "SELECT * FROM seeds_en WHERE cropname_en = '$cropname'";
                      } else {
                          $cropname = $district['cropname_te'];
                          $seeds = "SELECT * FROM seeds_te WHERE cropname_te = '$cropname'";
                      }
                  } else{
                      $cropname = $district['cropname_en'];
                      $seeds = "SELECT * FROM seeds_en WHERE cropname_en = '$cropname'";
                  }
                  $seedresult = $con->query($seeds);
                  if ($seedresult && $seedresult->num_rows > 0) {
                    echo '<div class="resp-tabs-container">';
                      echo '<div class="w3layouts_vertical_tab">';
                        foreach ($seedresult as $seed) {
                            echo '<div class="col-md-7 w3layouts_vertical_tab">';
                              echo "<p style='color:orange;'>". _SEEDS  . "</p>";
                              echo "<h3>" . $seed['seedname'] . "</h3>";
                              echo "<p style='color:orange;'>"._PRICE  . "<h4 style='color:black;'>" . '  '.  $seed['seedprice']. "</p></h4>";
                              echo "<p style='color:orange;'>"._COMMENTS  . "<h4 style='color:black;'><i>" . '  '. $seed['comments'] . "</i></h4></p>";
                            echo '</div>';
                            echo "<hr/>";
                            echo '<div class="col-md-5 w3layouts_vertical_tab_right">';
                              echo '<img width="150" height="150" src=../posts/'. $seed['file'] . ' class="img-responsive"';
                            echo '</div>';
                            echo "<hr/>";
                            echo '</div>';
                        }
