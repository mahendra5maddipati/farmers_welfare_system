...
<!-- banner -->
    ...
    <br/>
    <h1 style="color:white;font-size: 50px; text-shadow: 3px 4px 2px black;" class="mx-auto d-block text-center"><?= _EQUIPMENTS ?></h1>
	</div>
<!-- //banner -->
<!-- services -->
	<div class="services">
		<div class="container">
			<p class="w3ls_para"><?= _EQUIPMENTS ?></p>
      <br/>
      <div class="agileits_w3layouts_mail">
        <?php
          ...
          if(isset($_SESSION['lang'])){
              if ($_SESSION['lang'] == "en") {
                  $crops = "SELECT * FROM equipment_en";
              } else {
                  $crops = "SELECT * FROM equipment_te";
              }
          } else{
              $crops = "SELECT * FROM equipment_en";
          }
          $result = $con->query($crops);
          if ($result && $result->num_rows > 0) {
              $i = 0;
              foreach ($result as $crop) {
                  $i++;
                  echo '<div class="col-md-4 agileits_w3layouts_mail_left">';
                    echo '<div class="w3_agileits_mail_left">';
                      echo '<h4><i class="fa fa-suitcase" aria-hidden="true"></i>'. $crop['name'] . '</h4>';
                      echo '<div class="w3_agileits_mail_left1">';
                        echo '<span>'. _PRICE . '</span>'.'<p class="agileinfo_para" style="font-size:25px;">'. $crop['price'] .'</p><br/>';
                        echo '<p class="agileinfo_para">'. $crop['comments'] .'</p><br/>';
                      echo "</div>";
                    echo "</div>";
                  echo "</div>";
                  if ($i%3 == 0) {
